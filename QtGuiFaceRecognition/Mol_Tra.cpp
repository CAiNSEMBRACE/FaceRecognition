...
void Mol_Tra::modul_train(){
	//读取你的CSV文件路径.  
	//string fn_csv = string(argv[1]);  
	string fn_csv = "F:\\tianxu8822\\Learn\\Pri_Per_Lab3\\orl_faces\\train.txt";
	...
	if (images.size() <= 1) {
		...
		CV_Error(StsBadArg, error_message);
	}
...
void Mol_Tra::read_csv(const string& filename, vector<Mat>& images, vector<int>& labels, char separator) {
	...
	while (getline(file, line)) //从文本文件中读取一行字符，未指定限定符默认限定符为"\n"
	{
